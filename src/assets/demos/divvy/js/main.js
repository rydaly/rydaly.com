...
var D3LMap = {
  ...
  initMap: function () {
    ...
    D3LMap.g1 = D3LMap.svg
      .append("g")
      .attr("class", "leaflet-zoom-hide")
      .attr("id", "g1");
    D3LMap.g2 = D3LMap.svg
      .append("g")
      .attr("class", "leaflet-zoom-hide")
      .attr("id", "g2");
    D3LMap.g3 = D3LMap.svg
      .append("g")
      .attr("class", "leaflet-zoom-hide")
      .attr("id", "g3");
  ...
  initTripPaths: function () {
    d3.json(D3LMap.divvy_trips_json, function (collection) {
      ...
      collection.features.forEach(function (d, idx) {
        ...
        d.start_point = new L.LatLng(
          D3LMap.stations[d.from_station_id].lat,
          D3LMap.stations[d.from_station_id].lng
        );
        d.end_point = new L.LatLng(
          D3LMap.stations[d.to_station_id].lat,
          D3LMap.stations[d.to_station_id].lng
        );
        d.unique_id = idx;
        d.targ_circ_start_inner = d3.select(
          "#inner_circs_" + d.from_station_id
        );
        d.targ_circ_start_outer = d3.select(
          "#outer_circs_" + d.from_station_id
        );
    ...
    function convertToGeojson(data) {
      ...
      data.forEach(function (d, idx) {
        sort_array.push({
          id: data[idx].unique_id,
          start_time: data[idx].starttime.split(" ").pop(),
        }); // split and pop just the time off the end
      });
      ...
      for (var i = 0; i < sort_array.length; i++) {
        ...
        D3LMap.geojson.features[0].properties.targ_circ_start_inner.push(
          targStartInner
        );
        D3LMap.geojson.features[0].properties.targ_circ_start_outer.push(
          targStartOuter
        );
        D3LMap.geojson.features[0].properties.targ_circ_end_inner.push(
          targEndInner
        );
        D3LMap.geojson.features[0].properties.targ_circ_end_outer.push(
          targEndOuter
        );
  ...
  drawTripPaths: function (data) {
    ...
    feature.each(function (d, idx) {
      var theId = idx,
        start_latlng = new L.LatLng(
          data.features[0].geometry.coordinates[idx][0][1],
          data.features[0].geometry.coordinates[idx][0][0]
        ),
        end_latlng = new L.LatLng(
          data.features[0].geometry.coordinates[idx][1][1],
          data.features[0].geometry.coordinates[idx][1][0]
        ),
        start_circ_inner =
          data.features[0].properties.targ_circ_start_inner[idx],
        start_circ_outer =
          data.features[0].properties.targ_circ_start_outer[idx],
  ...
  animatePath: function (id, duration) {
    var test = d3
      ...
      .attr("style", function () {
        ...
        return (
          "pointer-events:none; transition: stroke-dashoffset " +
          dur +
          "s ease-in-out, opacity 2s " +
          dur +
          "s, stroke 2s " +
          dur +
          "s;"
    ...
    switch (phase) {
      case "incoming":
        ...
        parent_node
          .append("circle")
          .attr("cx", function () {
            return outer_node.attr("cx");
          })
          .attr("cy", function () {
            return outer_node.attr("cy");
          })
        ...
        innerTarget
          ...
          .attr("r", function () {
            return parseInt(innerTarget.attr("r")) + 3;
          });
        break;
  ...
  divvyTimer: {
    ...
    run: function () {
      ...
      this.node.innerText =
        (this.hour < 10 ? " " + this.hour : this.hour) +
        ":" +
      ...
      while (conv === dat.start_times[this.startTimeIndex]) {
        D3LMap.animatePulse(
          dat.targ_circ_start_outer[this.startTimeIndex],
          dat.targ_circ_start_inner[this.startTimeIndex],
          "outgoing",
          0
        );
        D3LMap.animatePulse(
          dat.targ_circ_end_outer[this.startTimeIndex],
          dat.targ_circ_end_inner[this.startTimeIndex],
          "incoming",
          dat.trip_durations[this.startTimeIndex] * 1.5
        );
        D3LMap.animatePath(
          this.startTimeIndex,
          dat.trip_durations[this.startTimeIndex]
        );
        this.startTimeIndex++;
      }
    },
