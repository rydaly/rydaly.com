...
const projectsEndpoint =
  "https://rydaly.com/cockpit/api/collections/get/projects?token=" +
  process.env.VUE_APP_COCKPIT_KEY;
const optionsEndpoint =
  "https://rydaly.com/cockpit/api/singletons/get/home_options?token=" +
  process.env.VUE_APP_COCKPIT_KEY;
...
let app = new Vue({
  ...
  created: function() {
    ...
    Promise.all(promises)
      .then(response => {
        let projectData = response[0].entries;
        let optionData = response[1];
        // console.log("projects :: ", response[0].entries);
